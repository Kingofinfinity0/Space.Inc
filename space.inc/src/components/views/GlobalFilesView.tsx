...
import { Briefcase, Download, Eye, File as GenericFile, FileSpreadsheet, FileText, History, Image as ImageIcon, Trash2, Upload, ArrowLeft, LayoutGrid, List, Search } from 'lucide-react';
...
const getDocumentTypeLabel = (file: SpaceFile) => {
    const mimeType = file.mime_type || file.type || '';
    const extension = getFileExtension(file);

    if (isImageFile(file)) return 'Image';
    if (mimeType.includes('pdf') || extension === 'pdf') return 'PDF';
    if (mimeType.includes('spreadsheet') || mimeType.includes('csv') || SPREADSHEET_EXTENSIONS.has(extension)) return 'Spreadsheet';
    if (mimeType.includes('presentation') || ['ppt', 'pptx', 'key'].includes(extension)) return 'Presentation';
    return 'Document';
};

const getDocumentTypeIcon = (file: SpaceFile, size = 16) => {
    const label = getDocumentTypeLabel(file);

    if (label === 'Image') return <ImageIcon size={size} className="text-sky-600" />;
    if (label === 'Spreadsheet') return <FileSpreadsheet size={size} className="text-emerald-600" />;
    if (label === 'PDF') return <FileText size={size} className="text-rose-600" />;
    return <GenericFile size={size} className="text-[#0D0D0D]" />;
};
...
const GlobalFilesView = ({ clients, profile }: { clients: ClientSpace[]; profile: any }) => {
    ...
    const { files: realtimeFiles, removeFile } = useRealtimeFiles('', organizationId || '', showTrash);
    const { showToast } = useToast();
    const documentFiles = useMemo(() => realtimeFiles.filter((file) => !isImageFile(file)), [realtimeFiles]);

    const groupedFiles = useMemo(() => clients.map((client) => ({
        client,
        files: documentFiles.filter((file) => file.space_id === client.id)
    })), [clients, documentFiles]);
    ...
    const filteredGroupedFiles = useMemo(() => groupedFiles.map((group) => ({
        ...
        files: group.files.filter((file) => {
            ...
            if (!q) return true;
            return [file.name, group.client.name, file.mime_type, getDocumentTypeLabel(file)].some((value) => String(value || '').toLowerCase().includes(q));
        })
    ...
    const totalTrash = documentFiles.filter((file) => file.deleted_at).length;
    ...
    const handleMoveToTrash = async (file: SpaceFile) => {
        if (!organizationId) return;
        if (!window.confirm(`Move ${file.name} to trash?`)) return;

        try {
            const { error } = await apiService.deleteFile(file.id, organizationId);
            if (error) throw error;
            removeFile(file.id);
            showToast('Document moved to trash.', 'success');
        } catch (err: any) {
            showToast(friendlyError(err?.message || 'Failed to move document to trash'), 'error');
        }
    };

    const handleRestore = async (file: SpaceFile) => {
        if (!organizationId) return;

        try {
            const { error } = await apiService.restoreFile(file.id, organizationId);
            if (error) throw error;
            removeFile(file.id);
            showToast('Document restored.', 'success');
        } catch (err: any) {
            showToast(friendlyError(err?.message || 'Failed to restore document'), 'error');
        }
    };

    const handleHardDelete = async (file: SpaceFile) => {
        if (!organizationId) return;
        if (!window.confirm(`Permanently delete ${file.name}? This cannot be undone.`)) return;

        try {
            const { error } = await apiService.hardDeleteFile(file.id, organizationId);
            if (error) throw error;
            removeFile(file.id);
            showToast('Document permanently deleted.', 'success');
        } catch (err: any) {
            showToast(friendlyError(err?.message || 'Failed to permanently delete document'), 'error');
        }
    };
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-col gap-3 md:flex-row">
                        <div className="relative min-w-[240px]">
                            ...
                            <Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search documents or spaces" className="rounded-[8px] pl-10 pr-4" />
                        </div>
            ...
            {viewMode === 'board' ? (
                            ...
                            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                                {files.length > 0 ? files.map((file) => (
                                    <div
                                        ...
                                        onKeyDown={(event) => {
                                            if (event.key === 'Enter') setViewingFile(file as any);
                                        }}
                                        className="database-row group flex cursor-pointer items-start justify-between gap-4 p-4 text-left"
                                    >
                                            ...
                                            {!showTrash ? (
                                                <>
                                                    <button
                                                        ...
                                                        onClick={(event) => {
                                                            ...
                                                            setViewingFile(file as any);
                                                        }}
                                                        className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#E5E5E5] bg-white text-[#6E6E80] hover:bg-[#F7F7F8] hover:text-[#0D0D0D]"
                                                        title="Preview"
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={(event) => {
                                                            event.stopPropagation();
                                                            void handleMoveToTrash(file);
                                                        }}
                                                        className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#F2D4D1] bg-white text-[#B42318] hover:bg-[#FFF4F2]"
                                                        title="Move to trash"
                                                ...
                                                <>
                                                    <button
                                                        ...
                                                        className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-emerald-200 bg-white text-emerald-700 hover:bg-emerald-50"
                ...
                <GlassCard className="sheet-panel overflow-hidden rounded-[8px]">
                    <div className="border-b border-[#E5E5E5] bg-[#F7F7F8] px-4 py-3">
                        <div className="grid grid-cols-[minmax(0,1.6fr)_140px_72px_160px_160px] gap-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6E6E80]">
                            <span>Name</span>
                            <span>Space</span>
                            <span>Type</span>
                            <span>Updated</span>
                    ...
                    <div className="divide-y divide-[#E5E5E5]">
                        {flatFiles.map(({ file, client }) => (
                            <div key={file.id} className="grid grid-cols-1 gap-4 px-4 py-4 md:grid-cols-[minmax(0,1.6fr)_140px_72px_160px_160px] md:items-center">
                                ...
                                <div className="flex items-center text-[#6E6E80]" title={getDocumentTypeLabel(file)} aria-label={getDocumentTypeLabel(file)}>
                                    {getDocumentTypeIcon(file, 18)}
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    {!showTrash ? (
                                        <>
                                            <Button variant="ghost" className="h-8 w-8 p-0 text-[#6E6E80] hover:text-[#0D0D0D]" onClick={() => setViewingFile(file as any)} title="Preview"><Eye size={16} /></Button>
                                            <Button variant="ghost" className="h-8 w-8 p-0 text-[#6E6E80] hover:text-[#0D0D0D]" onClick={() => void handleDownload(file)} title="Download"><Download size={16} /></Button>
                                            <Button variant="ghost" className="h-8 w-8 p-0 text-[#6E6E80] hover:text-[#0D0D0D]" onClick={() => setVersioningFile(file as any)} title="Version history"><History size={16} /></Button>
                                            <Button variant="ghost" className="h-8 w-8 p-0 text-[#B42318] hover:bg-[#FFF4F2] hover:text-[#B42318]" onClick={() => void handleMoveToTrash(file)} title="Move to trash"><Trash2 size={16} /></Button>
                                        </>
                                    ) : (
                                        <>
                                            <Button variant="ghost" className="h-8 w-8 p-0 text-emerald-700 hover:bg-emerald-50 hover:text-emerald-700" onClick={() => void handleRestore(file)} title="Restore"><ArrowLeft size={16} /></Button>
                                            <Button variant="ghost" className="h-8 w-8 p-0 text-[#B42318] hover:bg-[#FFF4F2] hover:text-[#B42318]" onClick={() => void handleHardDelete(file)} title="Delete permanently"><Trash2 size={16} /></Button>
                                        </>
                                    )}
                                </div>
                            </div>
