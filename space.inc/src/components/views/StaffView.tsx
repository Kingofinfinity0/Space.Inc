...
import { UserPlus, User, ShieldCheck, Sparkles, Search, LayoutGrid, List, Settings2 } from 'lucide-react';
      ...
      <GlassCard className="sheet-panel overflow-hidden p-6 md:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl space-y-3">
            ...
            <h1 className="text-3xl font-semibold tracking-[-0.04em] text-[#0D0D0D] md:text-4xl">Staff</h1>
            <p className="max-w-2xl text-sm leading-6 text-[#6E6E80] md:text-base">
              Manage staff assignments, active spaces, and access rules in a compact operational matrix.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full lg:max-w-md">
            ...
            <input
              ...
              className="w-full rounded-[8px] border border-[#DADADA] bg-white py-3 pl-10 pr-4 text-sm text-[#0D0D0D] placeholder:text-[#6E6E80] focus:border-black focus:outline-none"
            />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="surface-chip px-3 py-2 text-xs font-medium">
              <Settings2 size={14} /> Permission matrix
            </button>
            {(['all', 'owner', 'admin', 'staff'] as const).map((role) => (
              <button
                key={role}
                onClick={() => setRoleFilter(role)}
                className={`surface-chip px-3 py-2 text-xs font-medium capitalize ${roleFilter === role ? 'surface-chip-active' : ''}`}
              >
                {role}
              </button>
            ))}
            ...
            <GlassCard key={member.id} className="sheet-panel overflow-hidden p-5 md:p-6">
              <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
                <div className="flex min-w-0 items-start gap-4 md:gap-5">
                  ...
                  <div className="min-w-0 space-y-2">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {member.is_active ? (
                        <span className="surface-chip surface-chip-active px-2.5 py-1 text-[10px] uppercase tracking-[0.16em]">
                          <span className="indicator-dot" data-tone="green" />
                          Active
                        </span>
                      ) : (
                        <span className="surface-chip px-2.5 py-1 text-[10px] uppercase tracking-[0.16em]">Awaiting setup</span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="surface-chip px-3 py-1.5 text-[11px] uppercase tracking-[0.16em]">
