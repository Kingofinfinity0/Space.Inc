...
function TaskBadge({ priority }: { priority: NonNullable<Task['priority']> }) {
    return (
        <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-medium ${PRIORITY_STYLES[priority]}`}>
...
function TaskCard({
    ...
    return (
        <button
            ...
            onClick={onClick}
            className={`w-full rounded-2xl border border-zinc-200 bg-white p-4 text-left transition hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-[0_12px_30px_rgba(15,23,42,0.06)] ${compact ? 'min-h-[120px]' : 'min-h-[146px]'}`}
        >
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    <p className="line-clamp-2 text-sm font-medium text-zinc-950">{task.title}</p>
                    {task.description && <p className="mt-2 line-clamp-2 text-xs text-zinc-500">{task.description}</p>}
                </div>
        ...
        <div className={`grid gap-4 ${compact ? 'xl:grid-cols-2' : 'xl:grid-cols-4'}`}>
            {BOARD_COLUMNS.map((column) => {
                ...
                return (
                    <div
                        key={column.id}
                        className="flex min-h-[320px] flex-col rounded-[28px] border border-zinc-200/80 bg-[#f7f7f8] p-3"
                        onDragOver={(event) => event.preventDefault()}
                        onDrop={() => onDropTask(column.id)}
                    >
                        <div className="mb-3 flex items-center justify-between px-1">
                            <div>
                                <p className="text-sm font-medium text-zinc-900">{column.label}</p>
                                <p className="text-[11px] text-zinc-500">{items.length} tasks</p>
                            </div>
                            <span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-500">{items.length}</span>
                        </div>
                        <div className="flex flex-1 flex-col gap-3">
                            ...
                            {items.length === 0 && (
                                <div className="flex flex-1 items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-white/70 p-4 text-center text-xs text-zinc-400">
                                    Drop a task here
                                </div>
                            )}
                        </div>
            ...
            <div className="space-y-5">
                <div className={`flex flex-col gap-4 ${compact ? '' : 'xl:flex-row xl:items-end xl:justify-between'}`}>
                    <div className="space-y-2">
                        ...
                        <div>
                            <Heading level={compact ? 3 : 2} className={compact ? 'text-2xl md:text-3xl' : ''}>{title}</Heading>
                            <Text variant="secondary" className="mt-1 max-w-2xl">{subtitle}</Text>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-3">
                        <GlassCard className="min-w-[120px] px-4 py-3"><p className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">Open</p><p className="mt-1 text-2xl font-semibold text-zinc-950">{counts.open}</p></GlassCard>
                        <GlassCard className="min-w-[120px] px-4 py-3"><p className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">Due Soon</p><p className="mt-1 text-2xl font-semibold text-zinc-950">{counts.dueSoon}</p></GlassCard>
                        <GlassCard className="min-w-[120px] px-4 py-3"><p className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">Done</p><p className="mt-1 text-2xl font-semibold text-zinc-950">{counts.done}</p></GlassCard>
                        {allowCreate && <Button className="h-auto rounded-2xl px-5 py-3" onClick={openCreate}><Plus size={16} className="mr-2" />Add Task</Button>}
                    </div>
                </div>
                ...
                <GlassCard className="rounded-[30px] p-4 md:p-5">
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex flex-wrap gap-2">
                                {VIEW_OPTIONS.map((view) => (
                                    <button key={view.id} type="button" onClick={() => setActiveView(view.id)} className={`inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-medium transition-all ${activeView === view.id ? 'bg-zinc-950 text-white' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}>{view.icon}{view.label}</button>
                                ))}
                            </div>
                            <div className="flex flex-col gap-3 md:flex-row">
                                <div className="relative min-w-[240px]">
                                    <Search size={14} className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" />
                                    <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search task title or group" className="rounded-full pl-10 pr-4" />
                                </div>
                                <div className="flex gap-2">
                                    <select title="Filter by priority" value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value as typeof priorityFilter)} className="rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-700 outline-none">
                                    ...
                                    <select title="Filter by group" value={groupFilter} onChange={(event) => setGroupFilter(event.target.value)} className="rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-700 outline-none">
                        ...
                        {loading ? (
                            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-40 animate-pulse rounded-[28px] border border-zinc-200 bg-zinc-50" />)}</div>
                        ) : sortedTasks.length === 0 ? (
                            <div className="flex min-h-[280px] flex-col items-center justify-center rounded-[28px] border border-dashed border-zinc-200 bg-[#f7f7f8] px-6 py-12 text-center">
                                <SlidersHorizontal size={28} className="mb-4 text-zinc-300" />
                                <p className="text-base font-medium text-zinc-900">{emptyTitle}</p>
                                <p className="mt-2 max-w-sm text-sm text-zinc-500">{emptyDescription}</p>
                            </div>
                        ) : activeView === 'board' ? (
                            <TaskBoard tasks={sortedTasks} clients={clients} compact={compact} onCardClick={openEdit} onDragStart={setDraggingTaskId} onDropTask={(status) => void handleDrop(status)} />
                        ) : activeView === 'list' ? (
