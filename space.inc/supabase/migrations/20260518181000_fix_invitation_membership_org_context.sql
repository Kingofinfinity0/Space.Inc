-- Fix post-invitation routing context.
-- `get_my_contexts()` and `activate_membership_context()` require
-- `space_memberships.org_id`; accepted invitation rows and share-link rows must
-- carry it or the UI cannot transition into the target space.

UPDATE public.space_memberships sm
SET org_id = s.organization_id
...
BEGIN
  ...
  IF v_sm_id IS NULL THEN
    ...
    WHERE space_id = v_invite.space_id AND user_id = v_caller;
  END IF;
...
BEGIN
  ...
  IF v_sm_id IS NOT NULL THEN
    ...
    SET org_id = COALESCE(org_id, v_space_org_id), updated_at = now()
    WHERE space_id = v_link.space_id AND profile_id = v_caller;
    ...
    VALUES (
      ...
      'already_member',
      v_caller,
      jsonb_build_object('space_id', v_link.space_id, 'space_member_id', v_sm_id, 'org_id', v_space_org_id)
    );
  ...
  VALUES (
    ...
    'joined',
    v_caller,
    jsonb_build_object('space_id', v_link.space_id, 'space_member_id', v_sm_id, 'space_membership_id', v_smp_id, 'org_id', v_space_org_id)
  );
